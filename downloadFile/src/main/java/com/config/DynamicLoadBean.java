...
public class DynamicLoadBean implements ApplicationContextAware {
	 ...
	 *            要装载的bean所在的xml配置文件位置。 spring配置中的contextConfigLocation，
	...
	public void loadBean(String configLocationString) {
		...
		try {
			...
			ContextPropertyConfigurer propertyConfigurer = (ContextPropertyConfigurer) getApplicationContext()
					.getBean("propertyConfigurer");
			propertyConfigurer.postProcessBeanFactory(getApplicationContext().getBeanFactory());
